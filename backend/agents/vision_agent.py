...
class VisionAgent:
    ...
    2. Tone extraction (Ambient lighting-dependent approximation & Face-aware Monk/ITA v2)
    ...
    def check_ood(self, pil_image: Image.Image, skip_tier2: bool = False) -> Tuple[bool, str]:
        """
        ...
        # Tier 2: Deep Semantic AI Check (if API key available and not skipped)
        if not skip_tier2 and GEMINI_API_KEY:
            try:
    ...
    @staticmethod
    def _ita_angle(r: float, g: float, b: float) -> Optional[float]:
...
"""
    ...
    async def analyze(self, image_bytes: bytes, scan_mode: str = "normal") -> Dict[str, Any]:
        ...
        # 3. Acne & blemish assessment (audited 0.45 confidence threshold)
