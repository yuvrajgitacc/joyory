...
def test_ood_rejects_blank_tiny_and_accepts_skin():
    ok_blank, _ = vision_agent.check_ood(Image.new("RGB", (400, 400), (255, 255, 255)))
    ok_tiny, _ = vision_agent.check_ood(Image.new("RGB", (50, 50), (198, 152, 122)))
    ok_skin, _ = vision_agent.check_ood(skin_swatch())
    assert not ok_blank
    assert not ok_tiny
    assert ok_skin
